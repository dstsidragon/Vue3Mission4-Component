...
createApp({
    ...
    methods: {
        ...
        OneKeyDelAllProduct() {
            //將 取得全部商品 物件轉陣列 取出ID
            const allProductAry = Object.keys(this.productDataAll);
           console.log( allProductAry.length);

            allProductAry.forEach((item,i) => {
                // console.log(item)
                axios.delete(`${api_url}/api/${api_path}/admin/product/${item}`)
                    .then(
                        res => {
                            ...
                            if (res.data.success) {
                                // alert(`${res.data.message}`);
                                console.log(allProductAry.length == i+1)
                                if(allProductAry.length == i+1){
                                    this.getProduct();
